...
class Rule_p_imp_q:
    ...
    def process_logits(self, arg1):
        # scales logit with sigmoid or softmax depending on shape
        out = None
        if arg1.shape[1] == 1:
    ...
    def generic_interface(self, preds, rule_fxn_output):
        ...
        """
        # classes in order: ('tree'); already in [0, 1] so process logits not needed
        preds_canned =  preds  #self.process_logits(preds) 

        self.tree = preds_canned
...
class Rule_p_iff_q(Rule_p_imp_q):
    """
    all(x) p <==> q
    """
    def eval(self, p, q):
        p_imp_q = self.luk_imp(p, q)
        q_imp_p = self.luk_imp(q, p)
        out = self.luk_and(p_imp_q, q_imp_p)
        return out
