...
def get_transform(augment, ns_trans=False):
...
class NsTreeDataset(TreeDataset):
    ...
    def __init__(self,
        ...
        Returns:
            If train, path, image, targets else image
        """
        super().__init__(csv_file, root_dir, transforms, label_dict, train, preload_images)
        if transforms is None:
            self.transform = get_transform(augment=train, ns_trans=True)
        else:
            self.transform = transforms
    ...
    def __getitem__(self, idx):
        ...
        if self.preload_images:
            ...
            # chm stored on channel 4; seperate RGB and CHM
            temp_image = np.array(Image.open(img_name).convert("RGBA"))
            image_chm = temp_image[:, :, 3]
            
            image = temp_image[:, :, :3] / 255     # RGB
            image = image.astype("float32")
        ...
        if self.train:
            ...
            if np.sum(targets["boxes"]) == 0:
                ...
                image = np.rollaxis(image, 2, 0)
                image = torch.from_numpy(image)
                image_chm = torch.from_numpy(image_chm)  # chm is 1 channel
                targets = {"boxes": boxes, "labels": labels}
                return self.image_names[idx], image, targets
            ...
            return self.image_names[idx], image, image_chm, targets

        else:
            # Mimic the train augmentation
            converted = self.image_converter(image=image, chm=image_chm)
            return converted["image"]
